...
const LogoUpdate = () => {
  ...
  const fetchLogoData = async () => {
    Get(`${Api.LogoGetWebSite}?Id=${Id}`).then(handleResponse).then(processGet).catch(handleError);
  };
  ...
  const handleSubmit = (event) => {
    ...
    if (LogoTitles != '') {
      ...
      Update(`${Api.LogoGetWebSite}?Id=${Id}`, raw)
        .then(handleResponse)
        .then(processUpdate)
        .catch(handleError);
    } else {
  ...
  return (
    <Container>
      <Stack spacing={3} className="container mt-0">
        <title>Digi2l - follow Logo icon </title>
        <div className="m-3">
          <Stack spacing={3}>
