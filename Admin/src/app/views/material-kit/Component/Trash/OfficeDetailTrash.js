...
export default function OfficeDetailTrash() {
  ...
  var handleDeleted = async (Id) => {
    Deletes(`${Api.FooterDetailSite}?Id=${Id}`)
      .then(handleResponse)
      .then(processDelete)
      .catch(handleError);
  };
        ...
        if (row.DetailsisActive != true) {
          return (
            <>
              ...
              <IconButton>
                <Fab
                  onClick={() => OpeningsDeletes(row.Id)}
                  className="button democss"
                  aria-label="Delete"
                  variant="outlined"
                >
  ...
  useEffect(() => {
    var result = OpeningTabledata.filter((Opening) => {
      setSelectedRows([]);
      setToggleCleared(!toggleCleared);
      if (Opening.DetailsisActive == false) {
  ...
  const contextActions = React.useMemo(() => {
    const handleDeleteCheckBox = () => {
      ...
      if (Ids != null) {
        Restore(Ids);
        setOpeningTabledataFilter(differenceBy(OpeningTabledataFilter, 'Title'));
      }
    };
