...
updateProduct = async (req, res) => {
  try {
    ...
    const updatedAbb = await prisma.tblEnquiry.update({
      where: {
    ...
    res
      .status(200)
      .json({ Status: true, Message: "Update Success", result: updatedAbb });
  } catch (error) {
    res.status(400).json({ Status: false, Message: "Internal Server Error" });
  }
};
...
deleteProduct = async (req, res) => {
  try {
    const deletedAbb = await prisma.tblEnquiry.delete({
      where: {
        Id: Number(req.query.Id),
      },
    });
    res
      .status(200)
      .json({ Status: true, Message: "Delete Success", result: deletedAbb });
  } catch (error) {
    res.status(400).json({ Status: false, Message: "Internal Server Error" });
  }
};
