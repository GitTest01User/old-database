...
updateProduct = async (req, res) => {
  try {
    ...
    const updatedEnquiry = await prisma.tblEnquiry.update({
      where: {
    ...
    res
      .status(200)
      .json({ Status: true, Message: "Update Success", result: updatedEnquiry });
  } catch (error) {
...
deleteProduct = async (req, res) => {
  try {
    const deletedEnquiry = await prisma.tblEnquiry.delete({
      where: {
        Id: Number(req.query.Id),
      },
    });
    res
      .status(200)
      .json({ Status: true, Message: "Delete Success", result: deletedEnquiry });
  } catch (error) {
    res.status(400).json({ Status: false, Message: "Internal Server Error" });
  }
};
