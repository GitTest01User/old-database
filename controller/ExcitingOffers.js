    ...
    const mailOptions = {
      ...
      html: `Dear ${Name},<br><br/>
        Thank you for connecting with Digi2L- India’s First Digital
         Platform to sell used appliances. We have received your ${SourcePage} enquiry and
    ...
    const mailCustomercare = {
      ...
      html: `Dear Team <br><br/>
       ...
       Contact Person Name-${Name} 
            <br> Phone- ${Phone} 
            <br>Email- ${Email}
            <br> What Are You Looking For?- ${Looking}
            <br>Message- ${Message}
...
updateProduct = async (req, res) => {
  try {
    ...
    const updatedExciting = await prisma.tblEnquiry.update({
      where: {
    ...
    res
      .status(200)
      .json({ Status: true, Message: "Update Success", result: updatedExciting });
  } catch (error) {
...
deleteProduct = async (req, res) => {
  try {
    const deletedExciting = await prisma.tblEnquiry.delete({
      where: {
        Id: Number(req.query.Id),
      },
    });
    res
      .status(200)
      .json({ Status: true, Message: "Delete Success", result: deletedExciting });
  } catch (error) {
    res.status(400).json({ Status: false, Message: "Internal Server Error" });
  }
};
