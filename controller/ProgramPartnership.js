...
updateProduct = async (req, res) => {
  try {
    ...
    const updatedProgramPartership = await prisma.tblEnquiry.update({
      where: {
    ...
    res
      ...
      .json({
        ...
        result: updatedProgramPartership,
      });
  } catch (error) {
    res.status(400).json({ Status: false, Message: "Internal Server Error" });
  }
...
deleteProduct = async (req, res) => {
  try {
    const deletedProgramPartership = await prisma.tblEnquiry.delete({
      where: {
        Id: Number(req.query.Id),
      },
    });
    res
      ...
      .json({
        ...
        result: deletedProgramPartership,
      });
  } catch (error) {
    res.status(400).json({ Status: false, Message: "Internal Server Error" });
  }
