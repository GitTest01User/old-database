...
createProduct = async (req, res) => {
  try {
    ...
    data.append('message', 'Dear Customer - Thank you for registering your Enquiry on Digi2L. We will shortly get in touch with you. By UTC Digital Technologies.');
...
updateProduct = async (req, res) => {
  try {
    ...
    const updatedReseller = await prisma.tblEnquiry.update({
      where: {
    ...
    res
      .status(200)
      .json({ Status: true, Message: "Update Success", result: updatedReseller });
  } catch (error) {
    res.status(400).json({ Status: false, Message: "Internal Server Error" });
  }
};
...
deleteProduct = async (req, res) => {
  try {
    const deletedReseller = await prisma.tblEnquiry.delete({
      where: {
        Id: Number(req.query.Id),
      },
    });
    res
      .status(200)
      .json({ Status: true, Message: "Delete Success", result: deletedReseller });
  } catch (error) {
    res.status(400).json({ Status: false, Message: "Internal Server Error" });
  }
};
